...
: > "$OUT/report.tsv"
printf 'id\tslots\tstatus\twidth\theight\tfps\tduration\tcodec\tfile_url\tpage\n' >> "$OUT/report.tsv"
...
while read -r page slots; do
  ...
  ffmpeg -v error -y -i "$f" -vf "fps=1/$step,scale=480:-2,drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:text='%{pts\:hms}':x=8:y=8:fontsize=20:fontcolor=white:box=1:boxcolor=black@0.6,tile=4x3:padding=4:color=black" -frames:v 1 -q:v 4 "$OUT/${id}_sheet.jpg" || status=sheet_fail
